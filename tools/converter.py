...
parser.add_argument(
    ...
    required=True,
    type=str,
    help="The path of the exported tex file.",
)
parser.add_argument(
...
parser.add_argument(
    "--transpose", action="store_true", help="Whether to transpose the table."
)
...
method_names = sorted(
    list(set(chain(*[list(results[n].keys()) for n in dataset_names])))
)
...
for dataset_idx, dataset_name in enumerate(dataset_names):
    for metric_idx, ori_metric_name in enumerate(ori_metric_names):
        filled_value = (
            IMPOSSIBLE_UP_BOUND
            if ori_metric_name.lower() == "mae"
            else IMPOSSIBLE_DOWN_BOUND
        )
...
if not args.transpose:
    dataset_row = (
        ...
        + [
            style_templates["dataset_row_body"].format(
                num_metrics=num_metrics, dataset_name=x
            )
    ...
    # 构建第一列
    method_column = [
        style_templates["method_column_body"].format(method_name=x)
    ...
    for x in dataset_names:
        ...
        dataset_cell = [
            style_templates["dataset_column_body"].format(
                num_metrics=num_metrics, dataset_name=x
            )
        ]
    ...
    method_row = (
        ...
        + [
            style_templates["method_row_body"].format(method_name=x)
            for x in method_names
        ]
        + [r"\\"]
    )
    ...
    rows = [
        cells + row + [r"\\"] for cells, row in zip(additional_columns, ori_columns)
    ]
    rows = additional_rows + rows
